<template>
  <div>
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑证书' : '上传证书'" width="500px">
      <el-form :model="form" label-width="100px">
        ...
        <el-form-item label="类型">
          <el-select v-model="form.certificateType">
            ...
            <el-option label="质量文件" value="quality" />
          </el-select>
        </el-form-item>
...
const defaultForm = { titleCn: '', titleEn: '', imageUrl: '', descriptionCn: '', descriptionEn: '', certificateType: 'certificate', issueDate: '', sortOrder: 0, status: 1 }
const form = reactive({ ...defaultForm })

const loadList = async () => {
  loading.value = true
  try {
    const res: any = await certificateApi.list({ page: page.value, size: 20 })
...
const handleUpload = async (options: any) => {
  const res: any = await uploadApi.upload(options.file, 'certificate')
...
const handleSave = async () => {
  ...
  saving.value = true
  try {
    if (isEdit.value && editId.value) { await certificateApi.update(editId.value, form) }
    else { await certificateApi.create(form) }
